  ...
  return (
    <div>
      <div
        ...
        className="Corsor mt-4 flex w-full items-center justify-between rounded-3xl bg-[#FFF3F6] py-3 px-4"
      >
        <svg width="16" height="21" viewBox="0 0 16 21" fill="#655F5F" xmlns="http://www.w3.org/2000/svg">
        ...
        <span className="text-[11px] text-[#655F5F] md:text-[15px] tracking-[-2%]">
          How many rolls do I need? Use our <span className="underline">Calculator</span>
        </span>
        {isCalculatorVisible ? (
      ...
      {isCalculatorVisible && (
        <div className="mt-4 flex flex-col gap-4 rounded-md border bg-[#FFF3F6] p-2">
          <h2 className="text-center text-base font-medium text-[#000000]">How many rolls do I need?</h2>
          <div className="my-2 flex items-center justify-center gap-2">
            <label className="text-xs font-normal text-[#000000]">Feet</label>
            <label className="inline-flex cursor-pointer items-center">
              <input type="checkbox" checked={!isMeter} onChange={handleToggle} className="peer sr-only" />
              <div className={`peer relative h-5 w-9 rounded-full ${isMeter ? "bg-[#49AD91]" : "bg-[#49AD91]"} after:absolute after:start-[2px] after:top-[2px] after:h-4 after:w-4 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] ${!isMeter ? "peer-checked:after:translate-x-full peer-checked:after:border-white" : ""}`}></div>
            </label>
            <label className="text-xs font-normal text-[#000000]">Meter</label>
          </div>
          <div className="mx-auto my-2 w-full px-2 flex flex-col justify-center gap-4 md:flex-row">
            <div className="flex-1">
              <label className="text-xs font-normal uppercase text-[#000000]">Width</label>
              <div className="relative">
                <input type="number" onChange={(e) => setWidth(Number(e.target.value))} className="w-full border-white py-1 px-2" placeholder="0" />
                <span className="absolute right-2 top-0 flex h-full items-center text-gray-500">{isMeter ? "Ft" : "M"}</span>
              </div>
            </div>
            <div className="flex-1">
              <label className="text-xs font-normal uppercase text-[#000000]">Height</label>
              <div className="relative">
                <input type="number" onChange={(e) => setHeight(Number(e.target.value))} className="border-white w-full py-1 px-2" placeholder="0" />
                <span className="absolute right-2 top-0 flex h-full items-center text-gray-500">{isMeter ? "Ft" : "M"}</span>
              </div>
            </div>
          </div>
          <div className="flex justify-center">
            <div className="mt-4 w-1/2 rounded-full border-2 border-dashed border-pink-300 bg-white text-[#0B0A0A] py-[6px] text-xs text-center">
              <p className="mb-1">{`${width || 0} x ${height || 0} = ${rollsNeeded} rolls`}</p>
